...
Do not invent product scope or architecture. Do not implement (Local) or emit a Cloud paste until an answer key is approved and the app root has concrete verify commands. Prefer grill-me when decisions are still open and there is no app code; prefer grill-with-docs once a codebase exists.
